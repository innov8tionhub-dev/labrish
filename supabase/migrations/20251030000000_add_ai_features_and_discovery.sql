/*
  ...
  2. Schema Updates
    - Add visibility column to stories table (private/unlisted/public)
    - Add favorite_prompts JSONB column to user_preferences
...
-- Update stories table with visibility column
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE stories ADD COLUMN visibility text DEFAULT 'private' CHECK (visibility IN ('private', 'unlisted', 'public'));

    -- Migrate existing data
    UPDATE stories SET visibility = CASE
...
-- Add favorite_prompts to user_preferences
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_preferences' AND column_name = 'favorite_prompts'
  ) THEN
    ALTER TABLE user_preferences ADD COLUMN favorite_prompts jsonb DEFAULT '[]'::jsonb;
  END IF;
END $$;
...
-- Function to get user's monthly AI assist count
CREATE OR REPLACE FUNCTION get_user_ai_assist_count(
  p_user_id uuid,
  p_month text
)
RETURNS integer
...
BEGIN
  SELECT COUNT(*) INTO assist_count
  FROM ai_assist_usage
  WHERE user_id = p_user_id AND month = p_month;

  RETURN COALESCE(assist_count, 0);
END;
$$;
